  ...
  return (
    <Section id="projects" className="bg-transparent">
      <div className="mb-20">
        ...
        >
          Shipped for <span className="text-[var(--color-muted)]">enterprises &amp; startups.</span>
        </motion.h2>
      </div>
...
const CaseStudy = ({ study, index }: { study: CaseStudyData; index: number }) => {
  const { ref, x, y, onMouseMove, onMouseLeave } = useMagneticHover(0.06);
      ...
      <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
        <div className={`order-2 ${index % 2 === 0 ? 'lg:order-1' : 'lg:order-2'}`}>
          <div className="flex items-center gap-4 mb-6">
            ...
            <span className="text-[var(--color-muted)] font-mono text-sm uppercase tracking-wider">
              {study.category}
            </span>
          </div>
          ...
          <div className="space-y-8 mb-8">
            <div>
              <h4 className="text-sm font-bold text-[var(--color-muted)] uppercase tracking-wider mb-2">
                The Challenge
              </h4>
          ...
          <div className="border-l-2 border-[var(--color-border)] pl-6 mb-8">
            ...
            <ul className="space-y-2">
              {study.results.map((result, i) => (
                <li
                  key={i}
                  className="text-[var(--color-foreground)] font-medium flex items-center gap-3"
                >
        ...
        >
          <div className="relative aspect-[4/3] rounded-lg overflow-hidden bg-[var(--color-surface-highlight)] border border-[var(--color-border)] group-hover:border-primary/30 transition-colors duration-500">
            <div
              className={`absolute inset-0 bg-gradient-to-br ${study.color} opacity-10 mix-blend-overlay z-10`}
            ></div>
            <img
              src={study.image}
              alt={study.title}
